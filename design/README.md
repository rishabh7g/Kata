...
- **Accent text is `--color-accent-text`** (`accent-700`, 6.41:1 / 5.91:1). `--color-accent` is a **field** colour — rules and the 2px focus ring, and nothing else since #157 — and reads 3.76:1 on bg / 3.47:1 on surface: enough for non-text (SC 1.4.11, 3:1), never enough for a string (#71). Kickers, links, ghost labels, outline tags and the primary action's field all take the ink one.
...
- **Mark:** `assets/kata-mark.svg` — three steps climbing (ink) to an accent square: the TDD speed limit. The mark is final and is not re-drawn; what changed is only its gloss — the square used to be read as a Module's pass condition met, and the Library has no pass condition (#157), so the square is simply where the steps arrive. Ink + accent only; never recolor, never round.
...
- **Body grid:** `1fr 350px`, gap 56. Sections in the main column separated by 2px `.hr` rules: Concept Page (paragraphs, 66ch max) → Model Examples → Exercises. Every pack opens with an emphasis-only provenance line — `*LLM first draft · human-edited once · LLM novice pass · human-reviewed*` on m01–m05, `*LLM first draft · unedited*` on ai01–ai06 (#201) — and **the screen does not render it**: `stripConceptNote` (`src/screens/ModuleScreen.tsx`, #139) drops it before the body is parsed, so the Concept Page starts at its first real paragraph. The line stays committed in the markdown, where provenance belongs.
...
## Design tokens
`styles.css` (ship it) and `tokens.json` (mirror + app-layer values: layout grids, type scale, code sizes, semantics). Key semantics: red = emphasis and attention — primary action (field red `--color-accent`; any red *string* uses `--color-accent-text`), focus ring, rules. Never a green/red traffic pair, and no state semantic of any kind: the app reports nothing back about the reader or their code, so § semantics is down to `primaryAction`, `disabled` and `focus`. (`semantics.locked` is gone with the lock chain, #158. `semantics.failing` is gone with the read-only decision, #200 — it described how a screen would present a failed test run, and Kata reports no test results. `semantics.passed` is gone the same way, #204 — it described an ink-and-check marker plus an accent-field poster on a Module panel that #157 deleted, and named a term from `docs/ubiquitous-language.md` § Removed terms; nothing is submitted and nothing is passed, so there is no such state to style. `typeScale.app.bigRunNumber` and `typeScale.app.terminal` went with it, #204 — both sized the run-history block in the Exercise aside; so did `layout.exerciseGrid`, whose `1fr 400px` was that aside's column, and the Exercise screen has been one column since #157.)
...
- Icons: [Lucide](https://lucide.dev), 2px stroke, currentColor — used: **arrow-right** (Curriculum rows, Exercise cards) and **arrow-left** (the ghost back buttons). Those are the only two the app draws. `check` and `x` were the markers of a state the app no longer has, and went with the panels that showed them (#157, #204); they survive only in `DevGym.dc.html` and `screens/*.png`, prototype-only, like the rest of the historical material there.
