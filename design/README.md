...
- **Rows:** one per Module. Grid `104px 1fr 230px 36px`, gap 24, padding-block 24, 2px top rule per row + closing 2px rule after the last. Cells: ordinal (30px/800), title (22px, marked up `h2` — one level under the page `h1`) over one-line description (13px muted), status column (tag + optional `Checkpoint · date` 11px), trailing icon.
