...
- **Concept Page** — ~1 page of prose explaining the concept, written to `docs/01-design.md` § Editorial standard. The first draft is LLM-generated; how much human editing followed varies by pack, and the page's own first line — the provenance note (`docs/02-engineering.md` § 5), which no screen renders — is the record of it. Read-only content, no interaction.
...
- **ICurriculum** / **IProgress** — the two Target Interfaces the app is built from: reading the authored content, and owning the learner's Self-Check answers. Defined normatively in `docs/02-engineering.md` § 2.
